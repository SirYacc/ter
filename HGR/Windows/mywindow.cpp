...
MyWindow::MyWindow( string model, QWidget *parent )
    : QWidget( parent ),
      ...
      model( model )
{
   _mainLabel = new QLabel();
    ...
    labelOrder = new QLabel("Order : ");
    QString stlyeSheetlOrder = "QLabel{margin-top: 25px;font-size: 25px}";
    labelOrder->setStyleSheet(stlyeSheetlOrder);
    ...
    rightLayout->addWidget(labelOrder);
    ...
    QString lvlBat = "Batterie : " + QString::number(battery) + "%";
    batterie = new QLabel(lvlBat);
...
void MyWindow::setSecondaryLabel()
{
...
void MyWindow::setMainMat(const Mat &newmat)
{
    _mainMat = newmat;
    setMainLabel();
}

void MyWindow::setSecondaryMat(const Mat &newmat)
{
    _secondaryMat = newmat;
    setSecondaryLabel();
}

void MyWindow::setModeP() {
...
void MyWindow::inFlightMode( bool mode ) {
...
void MyWindow::inCalibrateMode( bool mode ) {
    buttonRP->setDisabled( mode );
    buttonRL->setDisabled( mode );
...
void MyWindow::setLanding() {
    order = 1;
    inFlightMode( false );
    context->setState( new DefaultWindowState() );
}

void MyWindow::onChangeCameraFeedClicked(){
    ihm = false;
    strategy->stopWaitForVideoStream();
}

void MyWindow::run( VideoStreamStrategy *strategy ) {
...
void MyWindow::waitForVideoStream() {

    setMainMat( blackCameraFeed );
    setSecondaryMat( blackSq );

    inWaitForVideoStream( true );
    strategy->waitForVideoStream();
    inWaitForVideoStream( false );
}

void MyWindow::inWaitForVideoStream( bool mode ) {
    buttonRP->setDisabled( mode );
    buttonRL->setDisabled( mode );
    buttonC->setDisabled( mode );
    buttonLand->setDisabled( mode );
...
void MyWindow::setInfoLabel(){
    QString lvlBat = "Batterie : " + QString::number(battery) + "%";
    batterie->setText(lvlBat);
    labelOrder->setText("Order : "+ orderName);
